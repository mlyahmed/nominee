			...
			t.Run("", func(t *testing.T) {
				...
				t.Logf("\tTest %d: When load configuration and %s.", i, example.description)
				{
					...
					if etcdConfig.Cluster != example.cluster {
						t.Fatalf("\t\t%s FAIL: Config.Cluster, expected <%s> but actual is <%s>", testutils.Failed, example.cluster, etcdConfig.Cluster)
					}
					t.Logf("\t\t%s Then the Config.Cluster should be loaded.", testutils.Succeed)

					if etcdConfig.Domain != example.domain {
						t.Fatalf("\t\t%s FAIL: Config.Domain, expected <%s> but actual is <%s>", testutils.Failed, example.domain, etcdConfig.Domain)
					}
					t.Logf("\t\t%s Then the Config.Domain should be loaded.", testutils.Succeed)

					if !reflect.DeepEqual(etcdConfig.Endpoints, strings.Split(example.endpoints, ",")) {
						t.Fatalf("\t\t%s FAIL: Config.Endpoints, expected <%s> but actual is <%s>", testutils.Failed, example.endpoints, etcdConfig.Endpoints)
					}
					t.Logf("\t\t%s Then the Config.Endpoints should be loaded.", testutils.Succeed)

					if etcdConfig.Username != example.username {
						t.Fatalf("\t\t%s FAIL: Config.Username, expected <%s> but actual is <%s>", testutils.Failed, example.username, etcdConfig.Username)
					}
					t.Logf("\t\t%s Then the Config.Username should be loaded.", testutils.Succeed)

					if etcdConfig.Password != example.password {
						t.Fatalf("\t\t%s FAIL: Config.Password, expected <%s> but actual is <%s>", testutils.Failed, example.password, etcdConfig.Password)
					}
					t.Logf("\t\t%s Then the Config.Password should be loaded.", testutils.Succeed)
				}
			})
		}
	}
		...
		for i, example := range invalidExamples {
			t.Run("", func(t *testing.T) {
				...
				t.Logf("\tTest %d: When load configuration and %s.", i, example.description)
				{
					defer func() {
						if r := recover(); r == nil {
							t.Fatalf("\t\t%s FAIL: Config.LoadConfig(). Expected the program to panic. Actual not.", testutils.Failed)
						} else {

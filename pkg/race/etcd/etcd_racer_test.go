	...
	t.Logf("Given EtcdRacer is stopped.")
	{
		for i, example := range examples {
			t.Run("", func(t *testing.T) {
				racer := etcd.NewEtcdRacer(example.config)
				defer racer.Cleanup()
				connector := etcd.NewMockServerConnector(t)
				racer.ServerConnector = connector

				t.Logf("\tTest %d: When Run and %s", i, example.description)
				{
					if err := racer.Run(service.NewMockService(t, &nominee.Nominee{})); err != nil {
...
func TestEtcdRacer_must_campaign_for_leadership(t *testing.T) {
	...
	{
		for i, example := range examples {
			srv := service.NewMockService(t, example.nominee)
			racer := etcd.NewEtcdRacer(example.config)
			connector := etcd.NewMockServerConnector(t)
			racer.ServerConnector = connector
			...
			t.Logf("\tTest %d: When Run and %s", i, example.description)
			{
				...
				if expected != actual {
					t.Fatalf("\t\t%s FAIL: EtcdRacer, expected conquer with value <%s> but actual is <%s>", testutils.Failed, expected, actual)
				}
...
func TestEtcdRacer_when_elected_then_promote_the_service(t *testing.T) {
	t.Logf("Given EtcdRacer is started.")
	{
		for i, example := range examples {
			srv := service.NewMockService(t, example.nominee)
			srv.LeadFn = func(context.Context, nominee.Nominee) error { return nil }
			...
			t.Logf("\tTest %d: When it is promoted and %s", i, example.description)
			{
				...
				if srv.LeadHits != 1 {
					t.Fatalf("\t\t%s FAIL: EtcdRacer, expected to promote the service. But actually not.", testutils.Failed)
				}
				t.Logf("\t\t%s The service is promoted.", testutils.Succeed)

				if string(leader.Kvs[0].Key) != srv.Leader.ElectionKey {
					t.Fatalf("\t\t%s FAIL: EtcdRacer, expected to promote the service itself. But actually not.", testutils.Failed)
				}
...
func TestEtcdRacer_when_err_on_promote_then_stonith(t *testing.T) {
	...
	{
		for i, example := range examples {
			mockService := service.NewMockService(t, example.nominee)
			etcdRacer := etcd.NewEtcdRacer(example.config)
			mockServerConnector := etcd.NewMockServerConnector(t)
...
func TestEtcdRacer_when_another_nominee_is_promoted_then_follow_it(t *testing.T) {
	...
	{
		for i, example := range examples {
			srv := service.NewMockService(t, &nominee.Nominee{Name: string(uuid.NodeID()), Address: uuid.NodeInterface()})
			srv.FollowFn = func(context.Context, nominee.Nominee) error { return nil }
			...
			t.Logf("\tTest %d: When another nominee is promoted and %s", i, example.description)
			{
				...
				t.Logf("\t\t%s Then the service is a follower.", testutils.Succeed)

				if string(leader.Kvs[0].Key) != srv.Leader.ElectionKey {
					t.Fatalf("\t\t%s FAIL: EtcdRacer, expected to follow the promoted service. But actually not.", testutils.Failed)
				}
...
func TestEtcdRacer_when_error_on_follow_then_stonith(t *testing.T) {
	t.Logf("Given the Service is bugged.")
	{
		for i, example := range examples {
			...
			srv.FollowFn = func(context.Context, nominee.Nominee) error {
...
func TestEtcdRacer_when_another_leader_replaces_it_then_stonith(t *testing.T) {
	t.Logf("Given EtcdRacer is the leader.")
	{
		for i, example := range examples {
			...
			srv.LeadFn = func(ctx context.Context, nominee nominee.Nominee) error { return nil }
